(() => {
  ...
  function teamNameFromPage(teamId){
    ...
    const names=[...document.querySelectorAll('a[href]')]
      .filter(anchor=>{try{return new URL(anchor.getAttribute('href'),location.origin).pathname.replace(/\/$/,'')===exact}catch{return false}})
  ...
  root.innerHTML=`
    ...
    <section id="fi-panel">
      ...
      <div id="fi-status" class="fi-status"><span class="fi-dot"></span><div><b>Extension connected</b><small>Yahoo is ready.</small></div></div>
      <div id="fi-stats" class="fi-stats" hidden>
        <div><b id="fi-teams">0</b><span>TEAMS</span></div>
        <div><b id="fi-players">0</b><span>ROSTERED</span></div>
        <div><b id="fi-matched">0</b><span>KEYED</span></div>
        <div><b id="fi-unmatched">0</b><span>UNKEYED</span></div>
      </div>
      <div class="fi-actions"><button id="fi-sync">SYNC YAHOO</button><button id="fi-open">OPEN FANTASY INTEL</button></div>
    ...
    <div id="fi-error-modal" hidden>
      <div class="fi-error-card">
        ...
        <div class="fi-error-sub">Copy this box if a Yahoo sync fails.</div>
        ...
        <div class="fi-error-actions"><button id="fi-copy">COPY ERROR</button><button id="fi-retry">RETRY</button><button id="fi-clear">CLEAR</button></div>
      </div>
    </div>`;

  const style=document.createElement('style');
  style.textContent=`
    #${ID}{position:fixed;left:18px;bottom:18px;z-index:2147483647;font-family:Inter,system-ui,-apple-system,sans-serif;color:#eef8ff}#${ID} *{box-sizing:border-box}
    ...
    #fi-panel{width:350px;border:1px solid #2b4765;background:linear-gradient(155deg,#07111f,#0d2138);border-radius:16px;padding:14px;box-shadow:0 18px 50px #0007}.fi-head{display:flex;justify-content:space-between;gap:10px}.fi-head small{color:#5fd8ff;font-size:9px;font-weight:900}.fi-head strong,.fi-head span{display:block}.fi-head strong{font-size:17px;margin-top:2px}.fi-head span{color:#95aac1;font-size:10px}.fi-head button{background:none;border:0;color:#9bb0c6;font-size:22px;cursor:pointer}
  ...
  async function resolveIdentity(){
    ...
    if(teamId){
      currentIdentity={leagueId:LEAGUE,teamId:String(teamId),teamName:teamNameFromPage(teamId)||data[key]?.teamName||`Yahoo Team ${teamId}`,detectedFrom:'url',updatedAt:new Date().toISOString()};
      await chrome.storage.local.set({[key]:currentIdentity,fantasyLeagueIdentity:currentIdentity});
    }else{
      currentIdentity=data[key]||data.fantasyLeagueIdentity||null;
    }
    document.getElementById('fi-team-label').textContent=currentIdentity?`${currentIdentity.teamName} · Team ${currentIdentity.teamId}`:'Open your Yahoo team page once to identify your team.';
  ...
  function showResult(result){
    ...
    const pool=result.yahooPlayersDetected?` · ${result.yahooPlayersDetected} Yahoo players read`:'';
    document.getElementById('fi-last').textContent=result.syncedAt?`Last sync: ${new Date(result.syncedAt).toLocaleString()}${pool}`:`Sync complete${pool}`;
  }
  ...
  function showError(diagnostic){
    ...
    modal.hidden=false;
    setStatus('Sync error','Diagnostic box is open.','error');
    expand();
  }
  ...
  async function runSync(){
    ...
    setStatus('Syncing Yahoo','Players, rosters and matchups are being refreshed.','syncing');
    try{
      ...
      setStatus('Sync complete',`${response.result.teams} teams · ${response.result.players} roster spots · ${response.result.yahooPlayersDetected||0} Yahoo players.`);
    }catch(error){
      showError({message:error.message,stage:'OVERLAY MESSAGE',page:location.href,version:VERSION,stack:error.stack});
    }finally{
      syncButton.disabled=false;
      syncButton.textContent='SYNC YAHOO';
    }
  }
  ...
  document.getElementById('fi-open').onclick=async()=>{
    await resolveIdentity();
    const response=await chrome.runtime.sendMessage({type:'OPEN_APP',leagueId:LEAGUE,teamId:currentIdentity?.teamId||''});
    if(!response?.ok)showError({message:response?.error||'Could not open Fantasy Intel.',stage:'OPEN APP',version:VERSION,page:location.href});
  };
  ...
  document.getElementById('fi-copy').onclick=async()=>{
    try{await navigator.clipboard.writeText(JSON.stringify(lastDiagnostic,null,2));document.getElementById('fi-copy').textContent='COPIED'}
    catch{document.getElementById('fi-copy').textContent='COPY FAILED'}
  };

  resolveIdentity();
  chrome.storage.local.get(['fantasyLeagueLastSync','fantasyLeagueLastError']).then(data=>{
    if(data.fantasyLeagueLastSync)showResult(data.fantasyLeagueLastSync);
    if(data.fantasyLeagueLastError)showError(data.fantasyLeagueLastError);
  });
